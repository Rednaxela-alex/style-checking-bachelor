    ...
    for document_path in corpora:
        ...
        for paragraph_index, paragraph in enumerate(paragraphs):
            ...
            if previous_para_embeddings is not None:
                ...
                del two_para_embeddings, two_para_lengths
        ...
        normalized_document_embeddings = normalize(document_embeddings, p=1.0, dim=1)
        normalized_paragraph_embeddings =  normalize(paragraphs_embeddings, p=1.0, dim=1)
        #### PREDICTIONS 
...
def my_embeddings_generater_for_training(dataset, input_path, output_path): 
    y = np.array([])
    X = torch.empty(0,768)
    for document_path in dataset:
        ...
        with open(document_path, encoding="utf8") as file:
            document= file.read()
        with open(ground_truth_document_path, encoding="utf8") as file:
            truth_document = file.read()
        ...
        for paragraph_index, paragraph in enumerate(paragraphs):
            ...
            for sentence in sentences:
                ...
                torch.add(current_para_embeddings, sentence_embedding, out=current_para_embeddings)                
                del sentence_embedding, sentence

            #print(current_para_embeddings)
            if previous_para_embeddings is not None:
                two_para_lengths= previous_para_length + current_para_length
                two_para_embeddings= (previous_para_embeddings + current_para_embeddings)/two_para_lengths
                paragraphs_embeddings.append(two_para_embeddings)            
        ...
        """
        try:
            X = torch.cat((X,paragraphs_embeddings), dim=0)
        except Exception as e:
            print(e)
